...
import { MagicEndpoint } from '../services/DataServices.js'
...
export default function MagicComponent() {
    ...
    const submitClick = async () => {
        if(numOne){
            const response = await MagicEndpoint();
            setResult(response);
        }
    };
  ...
  return (
    ...
    <header className="w-screen h-[8vh] bg-[#F5B553] flex">
        <h2 className="text-black font-bold font-amatic tracking-wide text-4xl my-auto ps-5 sm:ps-10 md:ps-20">AFO: Magic 8 Ball</h2>
    </header>

    <main className="w-[screen] h-[84vh] bg-[#854836] text-white px-10 sm:px-20 xl:px-50 py-14">
        <section className="h-full flex flex-col justify-between">
            <h1 className="font-amatic text-7xl font-bold text-center pb-5 pt-10"><span className="sm:hidden">Lvl.</span><span className="hidden sm:inline-block">Level</span> 8</h1>
            ...
            <div className="flex font-josefin font-bold">
                ...
                <div className="mx-auto">
                    <label className="block pb-5 text-2xl" htmlFor="input">Enter a question:</label>
                    <textarea onChange={e => setNumOne(e.target.value)} onKeyDown={handleKeyDown} className="text-black text-lg rounded-lg border-0 w-[230px] min-[375px]:w-[300px] sm:w-[400px]" id="input" rows="3" placeholder="What would you like to ask the magic 8 ball?" required></textarea>
                </div>
                <div className="mx-auto hidden xl:block">
